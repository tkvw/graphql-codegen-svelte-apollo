...
module.exports = {
    plugin: function (schema, documents, config, info) {
        ...
        var operationImport = ("" + (operations.some(function (op) { return op.operation == "query"; })
            ? "ApolloClient, ApolloQueryResult, ObservableQuery, WatchQueryOptions, " + (config.asyncQuery ? "QueryOptions, " : "")
        ...
        var ops = operations
            ...
            if (o.operation == "query") {
                ...
                if (config.asyncQuery) {
                    operation =
                        operation +
                            ("\n              export const Async" + o.name.value + " = (\n                options: Omit<\n                  QueryOptions<" + opv + ">,\n                  \"query\"\n                >\n              ) => {\n                return getClient().query<" + op + ">({query: " + (0, pascal_case_1.pascalCase)(o.name.value) + "Doc, ...options})\n              }\n            ");
                }
            }
